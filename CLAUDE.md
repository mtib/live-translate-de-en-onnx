...
- **Live hypothesis SSE.** `applyLifecycle` forwards `.partial(text:)` events to `liveAudioServer?.publishHypothesis(id:source:text:)` and `.completed`/`.dropped` events to `publishHypothesisDone(id:)`. These are named SSE events (`event: hypothesis` / `event: hypothesis-done`) on the `/events` stream only — never replayed in the session buffer. The ONNX backend reuses the chunk UUID at graduation (UUID continuity), so the JS listen page can upgrade the hypothesis row in-place when the final transcript arrives rather than inserting a duplicate row.

- **OBS overlay.** `/obs` serves a self-contained transparent-background HTML page suitable as a browser source in OBS (or any Chromium-based browser source tool). `/obs-events` is a dedicated SSE stream fed by `obsSubscribers` (a separate dict from the regular `subscribers`). German subtitle cues are sent as JSONL via `publishOBSLine`. On the page: subtitles stack from the bottom edge, each with an independent 4-second auto-clear timer, max 5 visible lines. `liveOBSURL` is `@Published` so the UI can surface it alongside the main stream URL. The server is always started unconditionally (not gated on TTS or language pair).
...
- [x] Live LAN stream — HTML listen page + open-ended WAV + SSE transcript + QR share
- [x] Live hypothesis fragments on mobile web page (rolling partial hypothesis with partial translation, in-place upgrade via named SSE events + UUID continuity)
- [x] Options window (font size, text colour, layout mode, window opacity) — Cmd+, opens `Settings` scene; gear button in menu-bar popover
- [x] OBS browser-source overlay — `/obs` transparent page, `/obs-events` SSE, English detection via `NLLanguageRecognizer`, en→de German subtitle stream via second `AppleTranslator`
